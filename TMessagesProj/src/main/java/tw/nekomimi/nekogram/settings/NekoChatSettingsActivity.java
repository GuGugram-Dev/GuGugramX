...
public class NekoChatSettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    private final AbstractConfigCell scrollableChatPreviewRow = cellGroup.appendCell(new ConfigCellTextCheck(GuGuConfig.INSTANCE.getScrollableChatPreview()));
            ...
            if (a instanceof ConfigCellTextCheck) {
                ...
                if (position == cellGroup.rows.indexOf(maxRecentStickerCountRow)) {
                    ...
                    builder.setItems(arrayList, (i, str) -> {
                        GuGuConfig.INSTANCE.getDoubleTapAction().setConfigInt(types.get(i));
                        listAdapter.notifyItemChanged(position);
                        return Unit.INSTANCE;
                    });
                    builder.show();
    ...
    private void showMessageMenuAlert() {
        ...
        for (int a = 0; a < count; a++) {
            ...
            switch (a) {
                ...
                case 3: {
                    textCell.setTextAndCheck(LocaleController.getString("RepeatAsCopy", R.string.RepeatAsCopy), GuGuConfig.INSTANCE.getShowRepeatAsCopy().Bool(), false);
                    break;
                }
                case 3 + 1: {
                    textCell.setTextAndCheck(LocaleController.getString("InvertReply", R.string.InvertReply), GuGuConfig.INSTANCE.getShowInvertReply().Bool(), false);
                    break;
                }
                case 4 + 1: {
            ...
            textCell.setOnClickListener(v2 -> {
                ...
                switch (tag) {
                    ...
                    case 3: {
                        textCell.setChecked(GuGuConfig.INSTANCE.getShowRepeatAsCopy().toggleConfigBool());
                        break;
                    }
                    case 3 + 1: {
                        textCell.setChecked(GuGuConfig.INSTANCE.getShowInvertReply().toggleConfigBool());
                        break;
                    }
                    case 4 + 1: {
                ...
                if (a instanceof ConfigCellCustom) {
                    ...
                    if (holder.itemView instanceof TextSettingsCell) {
                        ...
                        if (position == cellGroup.rows.indexOf(maxRecentStickerCountRow)) {
                            ...
                            textCell.setTextAndValue(LocaleController.getString("DoubleTapAction", R.string.DoubleTapAction), DoubleTap.doubleTapActionMap.get(GuGuConfig.INSTANCE.getDoubleTapAction().Int()), true);
                        }
                    }
                } else {
                    // Default binds
